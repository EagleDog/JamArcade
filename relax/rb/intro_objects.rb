...
class Knight < Chingu::GameObject
  def initialize(options)
    super
    @image = Gosu::Image["relax/assets/characters/knight.png"]
    @voice = Gosu::Sound["relax/assets/audio/mumble.ogg"]
...
class Sparkle < Chingu::GameObject
  def setup
    @image = Gosu::Image["relax/assets/intro/sparkle.png"]
    self.factor = 0.1
    @turning = 0.5
    @factoring = 1.0
    @angle = 35
...
class Highlight < Chingu::GameObject
  def setup
    @image = Gosu::Image["relax/assets/intro/highlight.png"]
  end
  def update
    @x += 5
  end
...
class Highlight2 < Chingu::GameObject
  def setup
    @image = Gosu::Image["relax/assets/intro/highlight2.png"]
  end
  def update
    @x += 5
  end
