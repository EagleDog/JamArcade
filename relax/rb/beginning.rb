...
class Test1 < Chingu::GameState
  trait :timer
  def setup
    $window.caption = "  TEST1"
    $window.width = 500
    after(200) { push_game_state(Test2)}
  end
end

class Test2 < Chingu::GameState
  trait :timer
  def setup
    $window.caption = "  TEST2"
    $window.width = 600
    after(1000) { push_game_state(Beginning)}
  end
end
...
#    puts "opening1"
  ...
  def draw
    Gosu::Image["relax/assets/intro/gosu-logo.png"].draw(0, 0, 0)
    super
  end
end
...
class Opening2 <  Chingu::GameState  #(Gamestate rescue Gosu::Window)
  ...
  def draw
    Gosu::Image["relax/assets/intro/ruby-logo.png"].draw(0, 0, 0)
    super
  end
end
...
class Introduction <  Chingu::GameState
  ...
  def setup
    puts "introduction"
    Chingu::Text.destroy_all # destroy any existing Text, Player, EndPlayer
    Player.destroy_all
...
#    $window.caption = "          ______ Relax ______"
    ...
    @click = Gosu::Sound["relax/assets/audio/pickup_chime.ogg"]
...
def update
  ...
  def draw
    Gosu::Image["relax/assets/intro/background.png"].draw(0, 0, 0)    # Background Image: Raw Gosu Image.draw(x,y,zorder)-call
    super
  end
end
