...
$sounds = {
  'pallet_ok': Gosu::Sample.new('boxes/sfx/423930__mudkip2016__correct.wav'),
  'box_ok': Gosu::Sample.new('boxes/sfx/476178__unadamlar__correct-choice.wav'),
  'box_nok': Gosu::Sample.new('boxes/sfx/478191__jonnyruss01__beep-error-1.ogg'),
  'box_missed': Gosu::Sample.new('boxes/sfx/521973__kastenfrosch__error.ogg'),
  'box_placed': Gosu::Sample.new('boxes/sfx/433770__zivs__ammo-pickup-1.ogg'),
  'alert': Gosu::Sample.new('boxes/sfx/529628__beetlemuse__alert-2.wav')
}
...
class Conveyor
  SPEEDS = [0, 0.5, 1, 4]
  CONTROL_PANEL = [
    Gosu::Image.new('boxes/gfx/conveyor_control_panel/stop.png', retro: true),
    Gosu::Image.new('boxes/gfx/conveyor_control_panel/speed_1.png', retro: true),
    Gosu::Image.new('boxes/gfx/conveyor_control_panel/speed_2.png', retro: true),
    Gosu::Image.new('boxes/gfx/conveyor_control_panel/speed_3.png', retro: true)
  ]
  ...
  def initialize(window, x, y, w, h)
    ...
    @ejection_spots = []
    @boxes = []
    @stripe = Gosu::Image.new('boxes/gfx/conveyor_stripe.png', retro: true)
    @offset_stripe = 0
    @speed = SPEEDS.first
  end
...
class Box
  ...
  WIDTH = 48
  HEIGHT = 32
  @@gfx = Gosu::Image.load_tiles('boxes/gfx/box.png', WIDTH, HEIGHT, retro: true)
  VELOCITY = 10.0
...
class Game1
  def initialize(window)
    ...
    @window = window
    @gfx = {
      title: Gosu::Image.new('boxes/gfx/title.png', retro: true),
      background: Gosu::Image.new('boxes/gfx/background.png', retro: true),
      tutorial: Gosu::Image.new('boxes/gfx/tutorial.png', retro: true)
    }

    setup_game

    @font = Gosu::Font.new(48)
    @music = Gosu::Song.new('boxes/sfx/funbgm102614.ogg')
  end
...
class Boxes < Chingu::GameState
  def initialize
    super #(800, 600, false)
    $window.width = 800
    $window.height = 600
    $window.cursor = true
    $window.caption = 'Boxes ! - Gosu Game Jam Oct 2021'
    @game = Game1.new(self)
  end
